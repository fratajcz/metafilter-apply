...
def main():
    ...
    if args.input_split_size > 0.000001:
        ...
        success = os.system("python3 utils/split.py --input {} --treats_identifier {} --seed {}".format(args.input, args.input_split_relation, args.input_split_seed))
        if not success:
            exit(1)
    ...
    if args.input[-7:] not in ["gpickle", "graphml"]:
        ...
        success = os.system("python3 utils/pickle_graph.py --input {} --sep {} --output {}".format(args.input, args.input_sep, input_base + ".gpickle"))
        if not success:
            exit(1)
    ...
    success = os.system("python3 metapathWalking.py --input {} --output {} --metapaths {} --njobs {} --nwalks {} --nstarts {}".format(
                                                     args.input, "walks.txt" , args.walking_metapaths, args.walking_njobs, args.walking_nwalks, args.walking_nstarts))
    if not success:
            exit(1)

    logger.info("Walking is done, uniquifying node ids.")
    success = os.system("python3 process_walks.py --input walks.txt --output unique_ids.txt".format(args.walking_njobs))
    if not success:
            exit(1)

    logger.info("Unique node ids obtained, modifying original graph.")
    success = os.system("python3 subset_graph.py --list unique_ids.txt --input {} --output {}".format(args.input, args.output))
    if not success:
            exit(1)
